import {
  defaultDataFormatter, defaultPrefix, defaultPrefixAndDataMergerForLevel,
  defaultPrefixFormatter, IFormatterOption, ILogger, ILoggerData, LogLevel
} from './common-logger';
// tslint:disable:no-console
// tslint:disable:member-ordering

import * as debug from '../common/log';
...
export class SimpleLogger implements ILogger {
  ...
  public info = (...msg: any[]) => {
    if (this.level === 'verbose') {
      ...
      formattedData.forEach((line) => debug.log(line));
    }
  }
  ...
  public warn = (...msg: any[]) => {
    if (this.level === 'verbose' || this.level === 'warn') {
      const formattedData = defaultPrefixAndDataMergerForLevel('warn', { ...this.formatterOption },
        this.formatPrefix, this.formatData, ...msg);
      formattedData.forEach((line) => debug.warn(line));
    }
  }
